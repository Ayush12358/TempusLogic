...
(defun p97 (array &optional (spot (next-undefined-spot array)))
  (if (null spot)
      (list array)
      (mapcan (lambda (n &aux (a (copy-seq array)))
                (setf (aref a (car spot) (cdr spot)) n)
                (p97 a (next-undefined-spot a spot)))
              (set-difference (p22 1 9)
...
(defun p98 (rows cols &optional puzzle)
  (if (null rows)
      (list puzzle)
